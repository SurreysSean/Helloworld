...
10.6 链表容器特定算法
> 1. list1.merge(list2,\<comp>) 合并list2到list1，清空list2，两者必须有序。可以额外传入谓词使用给定的比较操作；
> 2. l.sort(\<comp>)排序;l.reverse()翻转链表;l.unique()删除同一值的连续拷贝
> 3. splice成员
>> + lst.splice(p,lst2) p是迭代器。将lst2中所有元素以移动到lst中p之前的位置，清空lst2;两者不能是同一个链表
>> + lst.splice(p,lst2,p2) p,p2都是迭代器。将lst2中p2指向的元素移动到lst中；两个链表可以相同
>> + lst.splice(p,lst2,b,e) 将b,e表示lst2范围中的所有元素移动到lst中；两个链表可以但是p不能在b,e的范围内
